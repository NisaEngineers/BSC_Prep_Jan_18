...
# ────────────────────────────────────────────────
#  FIXED: Use user's home directory for persistent storage
# ────────────────────────────────────────────────
ROUTINE_FILE = os.path.expanduser("FinalRoutine/routine_state.json")
...
# Show where data is stored (helpful for debugging)
st.caption(f"📁 Data saved to: `{ROUTINE_FILE}`")
...
with analytics_tab:
    ...
    if all_data:
        ...
        # ------------------- NEW: Allocated vs Tracked Hours -------------------
        st.markdown("### Allocated vs Tracked Hours")
        # Compute per day planned hours and tracked hours
        day_summary = []
        for day_str, day_routine in routines.items():
            planned_min = 0
            actual_sec = 0
            for act, info in day_routine.items():
                # Planned duration from intervals
                for interval in info.get("intervals", []):
                    s_min = parse_minutes(interval["start"])
                    e_min = parse_minutes(interval["end"])
                    if e_min >= s_min:
                        dur = e_min - s_min
                    else:  # crosses midnight
                        dur = 1440 - s_min + e_min
                    planned_min += dur
                # Actual seconds from timer
                actual_sec += info.get("timer", {}).get("accumulated_seconds", 0)
            day_summary.append({
                "Date": day_str,
                "Planned Hours": planned_min / 60,
                "Tracked Hours": actual_sec / 3600
            })
        if day_summary:
            df_days = pd.DataFrame(day_summary)
            df_days["Date"] = pd.to_datetime(df_days["Date"])
            df_days = df_days.sort_values("Date")
            fig_comp = px.bar(df_days, x="Date", y=["Planned Hours", "Tracked Hours"],
                              barmode="group", title="Daily Planned vs Tracked Hours",
                              color_discrete_map={"Planned Hours": "blue", "Tracked Hours": "orange"})
            st.plotly_chart(fig_comp, use_container_width=True)

            # Optional summary metrics
            total_planned = df_days["Planned Hours"].sum()
            total_tracked = df_days["Tracked Hours"].sum()
            diff = total_tracked - total_planned
            col1, col2, col3 = st.columns(3)
            col1.metric("Total Planned Hours", f"{total_planned:.1f}")
            col2.metric("Total Tracked Hours", f"{total_tracked:.1f}")
            col3.metric("Difference", f"{diff:+.1f} hrs")
        else:
            st.info("No day data for comparison.")
        # -----------------------------------------------------------------------
    else:
