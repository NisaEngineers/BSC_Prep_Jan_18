...
for act, info in day_routine.items():
    ...
    info["completed"] = cols[1].checkbox("Done", value=info["completed"], key=comp_key)

    # Time inputs (5‑minute slots)
    info["start"] = cols[2].selectbox("Start", time_slots, index=time_slots.index(info["start"]), key=f"start_{day_str}_{act}")
    info["end"]   = cols[3].selectbox("End", time_slots, index=time_slots.index(info["end"]), key=f"end_{day_str}_{act}")
    ...
    # Notes
    info["notes"] = cols[5].text_area("Notes", info["notes"], key=f"notes_{day_str}_{act}")
